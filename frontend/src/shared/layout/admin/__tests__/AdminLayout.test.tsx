...
describe("AdminLayout", () => {
    it("redireciona para /login quando não há usuário logado", () => {
        mockedUseAuth.mockReturnValue({ user: null, isAuthenticated: false, login: vi.fn(), logout: logoutMock });
    ...
    it("mostra apenas os itens de menu permitidos para a role atendente", () => {
        ...
        expect(screen.queryByRole("link", { name: /caixa/i })).not.toBeInTheDocument();
    });
    ...
    it("exibe o nome do usuário logado e o rótulo da sua role", () => {
        mockedUseAuth.mockReturnValue({
            user: { id: "1", nome: "Marcos Ferreira", email: "marcos@x.com", role: "admin" },
            isAuthenticated: true,
            login: vi.fn(),
            logout: logoutMock,
        });
