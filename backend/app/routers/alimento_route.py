...
def criar_alimento(payload: AlimentoCreate, db: Session = Depends(get_db), current_user=Depends(require_admin)):
    return alimento_service.criar_alimento(db, payload, current_user.restaurant_id, current_user.id)
...
def atualizar_alimento(
    ...
    return alimento_service.atualizar_alimento(db, alimento_id, payload, current_user.restaurant_id, current_user.id)
...
def desativar_alimento(alimento_id: uuid.UUID, db: Session = Depends(get_db), current_user=Depends(require_admin)):
    alimento_service.desativar_alimento(db, alimento_id, current_user.restaurant_id, current_user.id)